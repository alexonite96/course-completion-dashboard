...
- [ ] **Step 4: Walk through the enablement-team flow in a browser**
  ...
  4. Click **Done**. Expect the stat tiles to populate and the table to show 3 hires (Jane, Kevin, Sofia) under the default 30-day window — "Old Hire Example" (45 days ago) should be excluded until you click **Show all**. **At this point every hire will show "Unmapped"** — `role_plan_mapping` starts empty (no seed rows), so no hire's Week 2 plan can be resolved yet. This is expected, not a bug: click **Manage role mapping** and add two rules (Role pattern `services consultant` → Learning Plan `Jump Start - Services Consultants Core & GL`; Role pattern `payroll specialist` → Learning Plan `Jump Start - Managed Payroll Specialist`), then close the panel. The table should now show Jane Cooper "On Track", Sofia Marin "Not Started", Kevin Adebayo "Behind".
  5. Click Jane Cooper's row. Expect the drawer to show Week 1 "Complete" and Week 2 "Complete" with real dates and course counts (1/1), not blank fields.
  6. Click **Show all**. Expect "Old Hire Example" to appear with status "Behind" (their JS Date is 45 days ago with zero completions, so Week 1 is overdue — this correctly outranks the "Unmapped" Week 2, since "Application Developer" doesn't match either mapping rule). Verify it renders without throwing.
